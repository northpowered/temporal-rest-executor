...
class PreDefinedEndpoint(BaseModel):
    type: str | None = None
    url: str | None = None
    name: str = "Name of this endpoint"
    description: str = "Description of this endpoint"
...
class PresetManifest(BaseSettings):
    ...
    def read_from_dict(self, raw_data: dict):
        self.endpoints = parse_obj_as(
            List[PreDefinedEndpoint], raw_data.get("endpoints", list())
        )
    ...
    def generate_router(self) -> APIRouter:
        ...
        for endpoint in self.endpoints:
            ...
            router.add_api_route(
                path=endpoint.url,
                endpoint=partial(prototype, client=client, payload=endpoint.config),
                dependencies=[client],
                methods=["post"],
                response_model=Any,
                name=endpoint.name,
